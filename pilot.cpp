...
std::vector<std::complex<double>> insertPilot(std::vector<std::complex<double>> &psdu, modParams& TxVector,  int n)
    {
        ...
        int psdu_length = psdu.size()/TxVector.Nsd; //PSDU length in terms of the OFDM symbols
        std::cout << "psdu_length " << psdu_length << std::endl;
        std::vector<std::complex<double> > data(TxVector.Nsa * psdu_length);
        std::vector<std::complex<double>>::iterator point = data.begin();
        std::complex<double> null_value = std::complex<double>(0, 0);

        switch (TxVector.channelWidth)
        {
            case(CBW2):
                    for(int s = 0; s < psdu_length; s++)
                    {
                       ...
                       for(int k = -32; k <= 31; k++)
                       {
                           ...
                           {
                               *point =psdu[(psdu_index++) + s*TxVector.Nsd];
                               point++;
                           }
                       }
